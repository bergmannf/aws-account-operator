...
func TestMoveAccount(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Dry run will prevent MoveAccount from being called", args: args{
			account:  "111111",
			client:   singleOrganisation(ctrl),
			targetOU: "targetOU",
			dryRun:   true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := MoveAccount(tt.args.account, tt.args.client, tt.args.targetOU, tt.args.dryRun); (err != nil) != tt.wantErr {
